...
> **🚀 Try the live demo:** Experience the payment flow in action at [https://a2a-kyapay-demo.skyfire.xyz](https://a2a-kyapay-demo.skyfire.xyz) — no setup required!
...
- Gemini API key (you can create one [here](https://ai.google.dev/gemini-api/docs/api-key))
...
### 4. View the Agent Card

Once the server is running, you can view the merchant agent's card at:

```
http://localhost:10000/agents/merchant_agent/.well-known/agent-card.json
```

The agent card declares the KYAPay extension and describes the agent's capabilities:
...
**Key Elements:**
- **`extensions`**: Declares support for the KYAPay payment protocol
- **`required: true`**: Indicates clients must understand KYAPay to interact with this agent
- **`skills`**: Describes the agent's monetized capabilities (selling datasets)
- **`url`**: The endpoint where the agent can be reached

### 5. Try the Payment Demo
...
|----------|-------------|----------|
| `GOOGLE_API_KEY` | Gemini API key for ADK agents | Yes |
