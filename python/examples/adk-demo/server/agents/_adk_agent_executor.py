...
class ADKAgentExecutor(AgentExecutor):
    ...
    async def _process_request(
        ...
        while True:
            ...
            for call in function_calls_to_execute:
                ...
                try:
                    # Execute the tool. This is where the KyaPaymentRequiredException will be raised.
                ...
                except KyaPaymentRequiredException:
                    # This special exception must propagate up to the KyaPayServerExecutor.
                    raise
    ...
    async def _preprocess_and_find_payment_payload(
        ...
        kya_payment_object and extracts the object's value.
        """
        for part in context.message.parts:
            ...
            if isinstance(part, DataPart):
                try:
                    ...
                    if isinstance(data, dict) and "kya_payment_object" in data:
                        # Return the base64 encoded payload string
                        return data["kya_payment_object"]
    ...
    async def execute(
        ...
        # Check if the kyapay wrapper has verified a payment by looking at the task metadata.
        if context.current_task and context.current_task.metadata.get(
            "kyapay_payment_verified", False
        ):
            # If payment is verified, write structured data to the session state.
            # The agent's `before_agent_callback` will read this.
            print(f"context.current_task.status.message.metadata: {context.current_task.status.message.metadata}")
            product_name = (
                context.current_task.status.message.metadata.get(
                    "kyapay.payment.required", {}  # NEW key
                )
                .get("accepts", [{}])[0]
                .get("description", "the item")  # NEW field - directly on accepts[0]
            )
            resource = (
                context.current_task.status.message.metadata.get(
                    "kyapay.payment.required", {}  # NEW key
                ...
                .get("accepts", [{}])[0]
                .get("resource", None)  # NEW field - directly on accepts[0]
            )
            session.state["payment_verified_data"] = {
                "product": product_name,
                "resource": DAPPIER_DATASETS[int(resource.split("/")[-1])]["dataUrl"],
                "status": "SUCCESS",
            }
            print(f"session payment verified data: {session.state['payment_verified_data']}")
            ...
            payment_data = session.state["payment_verified_data"]
            user_message = types.UserContent(
                parts=[types.Part(text=f"Payment verified. Payment data: {payment_data}")]
            )
            ...
            print(f"session state after upsert: {session.state}")
        else:
...
def convert_a2a_parts_to_genai(parts: list[Part]) -> list[types.Part]:
    """Convert a list of A2A Part types into a list of Gemini Gen AI Part types."""
    return [convert_a2a_part_to_genai(part) for part in parts]


def convert_a2a_part_to_genai(part: Part) -> types.Part:
    """Convert a single A2A Part type into a Gemini Gen AI Part type."""
    part = part.root
...
def convert_genai_parts_to_a2a(parts: list[types.Part]) -> list[Part]:
    """Convert a list of Gemini Gen AI Part types into a list of A2A Part types."""
    return [
...
def convert_genai_part_to_a2a(part: types.Part) -> Part:
    """Convert a single Gemini Gen AI Part type into an A2A Part type."""
    if part.text:
