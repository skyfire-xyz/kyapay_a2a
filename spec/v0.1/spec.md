...
This extension implements the [KYAPay protocol](https://www.kyapay.ai/), an open identity-linked payment protocol for agentic AI, within the A2A framework. KYAPay provides a standardized token format that embeds verified identity and payment authorization, enabling autonomous agents to transact securely across services.
...
**Learn More:** For a complete understanding of the underlying KYAPay protocol, including its vision, token structure, and broader ecosystem implications, visit [www.kyapay.ai](https://www.kyapay.ai/).
...
### 3.1. Complete Agent Card Example

Here's a complete example of an `AgentCard` for a merchant agent that accepts KYAPay payments:

```json
{
  "name": "KYAPay Merchant Agent",
  "description": "This agent sells items using the KYAPay payment protocol.",
  "url": "http://localhost:10000/agents/merchant_agent",
  "version": "5.0.0",
...
**Key Points:**
- The `extensions` array includes the KYAPay extension URI
- Setting `required: true` signals that clients MUST support KYAPay to use this agent
- The agent's `url` field points to where the agent can be reached
- Skills describe the monetized capabilities the agent offers
